 ...
 */
function BurgerCrossButton({ ref, responsive, position, color, isIconBurger, clickedOnce, onClick }: BurgerIconProps) {

  const strokeSize = 'h-[3px]';
  const strokeClassName = `${color} ${strokeSize} rounded-full w-full transition duration-500`;

  return (
    <button ref={ref} onClick={() => onClick()} className={`${responsive} ${position} group w-10 h-10 flex flex-col gap-3 cursor-pointer`}>
